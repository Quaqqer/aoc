...
def get_pos(val, i, rb):
    ...
    if mode == 0:
        ...
        return expr[i + val] + rb


def get_arg(val, i, rb):
    opstr = get_opstr(i)
    mode = int(opstr[2 - val])
    pos = get_pos(val, i, rb)
    return expr[pos]
...
def run():
    ...
    rb = 0
    while True:
        op = int(get_opstr(i)[-2:])

        # Exit
        if op == 99:
            print(f"Exit code: {expr[0]}")
            return
        ...
        # Addition
            ...
            arg1 = get_arg(1, i, rb)
            arg2 = get_arg(2, i, rb)
            store_pos = get_pos(3, i, rb)

            expr[store_pos] = arg1 + arg2
            i += increments[op]
