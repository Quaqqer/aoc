...
class Grid[T]:
    ...
    def mirror(self, axis: Literal["x"] | Literal["y"]) -> Grid[T]:
        match axis:
            case "x":
                return self[:, ::-1]
            case "y":
...
def print_set_grid(
    ...
    if isinstance(grid, set):
        ...
        def get(k: tuple[int, int]):
            return "█" if k in grid else empty
    else:

        def get(k: tuple[int, int]):
            return str(grid.get(k, empty))
...
def print_grid(grid: Sequence[Sequence[str | int]], transpose: bool = False):
    if not transpose:
        for line in grid:
...
class Vec2[T: (int, float)]:
    ...
    def __init__(self, x: T | tuple[T, T], y: T | None = None):
        match (x, y):
            ...
            case (x, y):
                self.__x: T = cast(T, x)
                self.__y: T = cast(T, y)
            case p:  # pyright: ignore[reportUnnecessaryComparison]
                assert_never(p)
    ...
    def rot_r(self, y_down: bool = True) -> Vec2[T]:
        if not y_down:
            return Vec2(self.y, -self.x)

        return Vec2(-self.y, self.x)

    def rot_l(self, y_down: bool = True) -> Vec2[T]:
        if not y_down:
            return Vec2(-self.y, self.x)
    ...
    @staticmethod
    def delta_4() -> tuple[Vec2[int], ...]:
        return (Vec2(1, 0), Vec2(0, 1), Vec2(-1, 0), Vec2(0, -1))

    @staticmethod
    def delta_8() -> tuple[Vec2[int], ...]:
        return (
            Vec2(1, 0),
            Vec2(1, 1),
            Vec2(0, 1),
...
def chunk[T](
    iterable: Iterable[T], size: int, throw_on_cut: bool = True
) -> Iterable[tuple[T, ...]]:
    l: list[T] = []
